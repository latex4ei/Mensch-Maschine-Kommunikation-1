...
\begin{sectionbox}
	...
	\vspace{-1em}\paragraph{Endliche Zustandsautomaten}
	Automat mit endlicher Menge von Zuständen $S$

	\vspace{-1em}\paragraph{Deterministische Zustandsautomaten}
	Automat, in dem die Folgezustände immer \emph{eindeutig} definiert sind durch den aktuellen Zustand und Eingabesymbol \\
...
\begin{sectionbox}
	...
	\begin{itemize}
		...
		\item (Für $\mathcal S$, $\mathcal X$, $\ma T$, $s_0$ siehe Zustandsautomat)
	\end{itemize}

	\vspace{-1em}\paragraph{Aktionen:}
	\begin{itemize}
		\item $\text{push}(x)$ lege $x$ auf den Stack
		\item $\text{pop}()$ lese und entferne oberste Stack-Element
		\item $\varepsilon$ keine Aktion
		\item $\#$ leeres Stack (\emph{erster} und \emph{letzter} Schritt)
	\end{itemize}
	...
	(Voraussetzung auf Stack $\in \mathcal Y$, Eingabe $\in \mathcal X$, Aktion)
\end{sectionbox}
...
\begin{sectionbox}
	...
	\begin{equation*}
	\begin{aligned}
		\ma C_k &= \left( \frac{1}{M_k} \sum\limits^{M_k}_{i=1} \vec r_{k,i} \cdot r^T_{k,i} - m_k \cdot m_k^T \right) \\
		\ma W_k &= \ma C_k^{-1} \\
